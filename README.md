...
The configuration is organized with the [Dendritic Pattern](https://github.com/mightyiam/dendritic) on top of [flake-parts](https://flake.parts): every `.nix` file under `modules/` is a feature, auto-imported by `import-tree`, and hosts (`modules/hosts/`) and users (`modules/users/`) are themselves features composed of smaller ones. `flake.nix` is auto-generated by [flake-file](https://github.com/vic/flake-file) — declare inputs in a feature's `flake-file.inputs` and run `nix run .#write-flake`. See `.claude/skills/nix-config/` for the full guide.
