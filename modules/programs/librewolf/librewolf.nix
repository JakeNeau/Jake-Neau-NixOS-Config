    ...
    let
      ...
      browserExecutable =
        if pkgs.stdenv.hostPlatform.isDarwin then
          "${config.programs.librewolf.finalPackage}/Applications/LibreWolf.app/Contents/MacOS/librewolf"
        else
          ...
          ''
            ...
            ${lib.concatMapStringsSep "\n" (
              ...
              let
                ...
                badgeFill =
                  if solid then "xc:${badge.darkColor}" else "gradient:${badge.lightColor}-${badge.darkColor}";
                baseIcon =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    "work/source/firefox_${toString macSourceSize}x${toString macSourceSize}x32.png"
                  else
                    "${config.gtk.iconTheme.package}/share/icons/${config.gtk.iconTheme.name}/apps/scalable/librewolf.svg";
      ...
      profileDesktopEntries = lib.mapAttrs' (
        profile: launcher:
        lib.nameValuePair (if profile == "work" then "librewolf" else "librewolf-${profile}") {
          name = profileDisplayNames.${profile};
          genericName = "Web Browser";
          exec = "${lib.getExe launcher} %U";
          icon = if profile == "work" then "librewolf" else "librewolf-${profile}";
          terminal = false;
          categories = [
            "Network"
            "WebBrowser"
    ...
    lib.mkMerge [
      {
        ...
        home.packages = profileLauncherPackages ++ profileIconPackages ++ macOSProfileApps;
      }
