  ...
  flake.modules.homeManager.nvf =
    ...
    in
    {
      programs.nvf.settings.vim = {
        ...
        globals = {
          # nixfmt's default is 2. nvf passes the buffer width to it, so this only
          # applies where no project declares one.
        ...
        luaConfigRC.indentRepair = ''
          local excluded = ${"{" + lib.concatMapStringsSep ", " (ft: "${ft} = true") treesitterExcludes + "}"}
        ...
        formatter.conform-nvim.setupOpts.formatters =
          let
            ownArgs =
              name: lib.mkForce (lib.generators.mkLuaInline "require('conform.formatters.${name}').args");
          in
          {
