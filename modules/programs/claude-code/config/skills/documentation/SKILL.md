...
the project's local style. Load [[skill:writing]] unless the task already loaded
it.

After changing Markdown or plain text, run this command:

```sh
claude-writing-lint <changed-file>
```

Resolve valid findings. Report heuristic exceptions instead of weakening
accurate content. Run the project's documentation build or checks when they
exist.

## Related skills

- [[skill:writing]]: route the prose task and order substance before form
