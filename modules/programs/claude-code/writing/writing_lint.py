...
def main(argv=None, stdin=None, stdout=None, stderr=None):
    ...
    parser = argparse.ArgumentParser(prog="claude-writing-lint")
    ...
    if args.files:
        for filename in args.files:
            ...
            if path.suffix.lower() not in {".md", ".markdown", ".txt"}:
                stderr.write(
                    f"claude-writing-lint: unsupported file type: {filename}\n"
                )
                return 2
            try:
                text = path.read_text()
                ...
                stderr.write(
                    f"claude-writing-lint: cannot read {filename}: {error}\n"
                )
                return 2
            diagnostics.extend(
