...
let
  ...
  wrapValue =
    ...
    if v ? _type then
      if v._type == "if" then
        v // { content = wrapValue v.content; }
      else if v._type == "merge" then
        v // { contents = map wrapValue v.contents; }
    ...
    else if lib.isAttrs v && v ? outPath && !lib.isDerivation v then
      v
    else if lib.isAttrs v && !lib.isDerivation v then
      lib.mapAttrs (_: wrapValue) v
  ...
  wrapConfig =
    ...
    if c ? _type then
      if c._type == "if" then
        c // { content = wrapConfig c.content; }
      else if c._type == "merge" then
        c // { contents = map wrapConfig c.contents; }
      else if c._type == "override" then
        c
      else
        lib.mkOverride 900 c
    else if lib.isAttrs c && !lib.isDerivation c then
      lib.mapAttrs (n: v: if lib.elem n configPlumbingKeys then v else wrapValue v) c
    else
      lib.mkOverride 900 c;
  ...
  wrapModule =
    ...
    else if lib.isFunction m then
      lib.mirrorFunctionArgs m (args: wrapModule (m args))
    ...
    then
      ...
      lib.mapAttrs (
        ...
        if n == "imports" then
          map wrapModule v
        else if lib.elem n (moduleMetaKeys ++ configPlumbingKeys) then
          v
        else
          wrapValue v
      ) m;
  ...
  generate =
    ...
    let
      ...
      ways =
        program:
        if config.flake.programs ? ${program} then
          config.flake.programs.${program}.install.${platform}
        else
          fail "requests program \"${program}\", which has no flake.programs declaration; declare it or drop the entry";
      ...
      wayOf =
        ...
        if override != null then
          if lib.elem override declared then
            override
          else
            fail "installOverrides.${program} = \"${override}\", but ${program} declares only ${builtins.toJSON declared} on ${platform}; pick a declared way or drop the override"
        else if lib.elem "home" declared then
          "home"
        else if declared != [ ] then
          lib.head declared
        else
          fail "lists ${program} in globalPrograms, but it declares no install way on ${platform}; add one to flake.programs.${program}.install or drop the entry";
      ...
      systemUnits = map (g: inputs.self.modules.${host.class}.${g.program}) (
        lib.filter (g: g.way != "home") globals
      );
      baselineUnits = map (
        g: inputs.self.modules.homeManager.${if g.way == "home" then g.program else "${g.program}-config"}
      ) globals;
      ...
      hostConfig = {
        ...
        imports = map (u: inputs.self.modules.${host.class}.${u}) host.users ++ systemUnits;
      };
    in
  ...
  stampSystem =
    ...
    if host.class == "nixos" then
      ...
      inputs.nix-darwin.lib.darwinSystem { inherit modules; };

  systemsFor =
    class: lib.mapAttrs stampSystem (lib.filterAttrs (_: host: host.class == class) config.flake.hosts);
