...
let
  ...
  generate =
    ...
    let
      ...
      homePlatforms = lib.filter (p: lib.elem "home" decl.install.${p}) [
        "linux"
        "macos"
      ];
      ...
      units =
          ...
          homeManager = {
            ${name} =
              if hasHome then
                ...
                tombstone name "declares no per-user install way; import homeManager.${name}-config for the config alone, or add \"home\" to an install.<platform> list";
            # Config only, carried plain — for users on machines where the
            # install is global/system-level.
            "${name}-config" =
              if decl.hasEnableOption && (hasSystem || hasCask) then
                {
                  imports = [ decl.config ];
        ...
        // lib.optionalAttrs (!suppressed "nixos") {
          nixos.${name} =
            if hasSystem then
              { pkgs, ... }: { environment.systemPackages = [ pkgs.${name} ]; }
            else
              tombstone name "declares no Linux \"system\" install way; import homeManager.${name}, or add \"system\" to install.linux";
        }
        // lib.optionalAttrs (!suppressed "darwin") {
          darwin.${name} =
            if hasCask then
              ...
              tombstone name "declares no macOS \"cask\" install way; import homeManager.${name}, or add \"cask\" to install.macos";
        };
    in
    # Incoherent declarations fail here, at declaration eval, before any
    # unit is imported anywhere.
    if decl.install.linux == [ ] && decl.install.macos == [ ] then
      fail "declares no install way on any platform; set install.linux and/or install.macos, or delete the declaration"
    else if hasHome && suppressed "homeManager" then
      fail "declares a per-user (\"home\") install way but marks homeManager hand-written; drop the way or the handWritten entry"
    else if hasSystem && suppressed "nixos" then
      fail "declares the Linux \"system\" install way but marks nixos hand-written; drop the way or the handWritten entry"
    else if hasCask && suppressed "darwin" then
      fail "declares the macOS \"cask\" install way but marks darwin hand-written; drop the way or the handWritten entry"
    else
      units;
in
{
